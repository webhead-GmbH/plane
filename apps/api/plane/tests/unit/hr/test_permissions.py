...
class TestContextResolution:
    def test_an_ordinary_employee_is_not_a_manager(self, workspace):
        ...
        resolved, is_manager = resolve_hr_context(request_for(user), workspace.slug)
    ...
    def test_the_flag_makes_someone_a_manager_without_touching_their_workspace_role(
        ...
        _, is_manager = resolve_hr_context(request_for(user), workspace.slug)
        assert is_manager is True
        assert (
            WorkspaceMember.objects.get(workspace=workspace, member=user).role == ROLE.MEMBER.value
        )

    def test_a_workspace_administrator_is_a_manager(self, workspace):
        # They can already grant themselves the flag, so withholding it buys
        # nothing; what matters is that the access is recorded.
        _, is_manager = resolve_hr_context(request_for(workspace.owner), workspace.slug)
        assert is_manager is True

    def test_an_administrator_without_an_employment_record_still_gets_through(self, workspace):
        # Administering the workspace does not require being employed through it.
        profile, is_manager = resolve_hr_context(request_for(workspace.owner), workspace.slug)
        assert profile is None
        assert is_manager is True

    def test_somebody_outside_the_workspace_gets_nothing(self, workspace):
        outsider = new_user()
        profile, is_manager = resolve_hr_context(request_for(outsider), workspace.slug)
        assert profile is None
        assert is_manager is False

    def test_a_deactivated_member_gets_nothing(self, workspace):
        user, _ = add_person(workspace)
        WorkspaceMember.objects.filter(workspace=workspace, member=user).update(is_active=False)
        profile, is_manager = resolve_hr_context(request_for(user), workspace.slug)
        assert profile is None
        assert is_manager is False
...
class TestVisibility:
    def test_an_employee_sees_only_themselves(self, workspace):
        ...
        add_person(workspace)
        visible = visible_profiles(prepared(user, workspace.slug), workspace.slug)
    ...
    def test_a_manager_sees_everyone(self, workspace):
        ...
        add_person(workspace)
        add_person(workspace)
        visible = visible_profiles(prepared(manager_user, workspace.slug), workspace.slug)
        assert visible.count() == 3

    def test_visibility_does_not_leak_across_workspaces(self, workspace):
        manager_user, _ = add_person(workspace, is_hr_manager=True)
    ...
    def test_an_outsider_sees_nothing(self, workspace):
        add_person(workspace)
        outsider = new_user()
        visible = visible_profiles(prepared(outsider, workspace.slug), workspace.slug)
    ...
    def test_an_employee_cannot_read_a_colleague_by_asking_for_them_directly(self, workspace):
        ...
        found = readable_profile_or_none(prepared(user, workspace.slug), workspace.slug, colleague.id)
        assert found is None

    def test_a_manager_can_read_a_colleague_directly(self, workspace):
        manager_user, _ = add_person(workspace, is_hr_manager=True)
        _, colleague = add_person(workspace)
        found = readable_profile_or_none(
            prepared(manager_user, workspace.slug), workspace.slug, colleague.id
        )
        assert found == colleague

    def test_asking_for_nobody_in_particular_returns_your_own_record(self, workspace):
        user, profile = add_person(workspace)
        found = readable_profile_or_none(prepared(user, workspace.slug), workspace.slug, None)
...
class TestApproval:
    def test_an_employee_cannot_approve(self, workspace):
        user, profile = add_person(workspace)
        assert can_approve(prepared(user, workspace.slug), profile) is False

    def test_a_manager_can_approve_for_someone_else(self, workspace):
        manager_user, _ = add_person(workspace, is_hr_manager=True)
        _, colleague = add_person(workspace)
        assert can_approve(prepared(manager_user, workspace.slug), colleague) is True
    ...
    def test_a_manager_cannot_approve_their_own(self, workspace):
        ...
        assert can_approve(prepared(manager_user, workspace.slug), manager_profile) is False
