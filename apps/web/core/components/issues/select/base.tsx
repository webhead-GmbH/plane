...
type TWorkItemLabelSelectOptionsProps = {
  createLabelEnabled: boolean;
  filteredOptions: IIssueLabel[];
  handleAddLabel: (labelName: string) => Promise<void>;
  labelsList: IIssueLabel[];
  query: string;
  submitting: boolean;
};

const WorkItemLabelSelectOptions = observer(function WorkItemLabelSelectOptions(
  props: TWorkItemLabelSelectOptionsProps
) {
  const { createLabelEnabled, filteredOptions, handleAddLabel, labelsList, query, submitting } = props;
  // plane hooks
  const { t } = useTranslation();

  if (!labelsList || !filteredOptions) return <p className="px-1.5 py-1 text-placeholder italic">{t("loading")}</p>;
  ...
  if (submitting) return <LoadingOutline className="h-3.5 w-3.5 animate-spin" />;

  if (!createLabelEnabled) return <p className="px-1.5 py-1 text-placeholder italic">{t("no_matching_results")}</p>;
