...
function ActiveCycleStatsLoader() {
  return (
...
type ActiveCyclePriorityIssuesProps = Pick<
  ActiveCycleStatsProps,
  "workspaceSlug" | "projectId" | "handleFiltersUpdate" | "cycleIssueDetails"
> & {
  setIssueLoaderElement: (element: HTMLDivElement | null) => void;
  emptyStateAssetPath: string;
};

const ActiveCyclePriorityIssues = observer(function ActiveCyclePriorityIssues(props: ActiveCyclePriorityIssuesProps) {
  const {
    workspaceSlug,
    projectId,
    handleFiltersUpdate,
    cycleIssueDetails,
    setIssueLoaderElement,
    emptyStateAssetPath: priorityResolvedPath,
  } = props;
  // plane hooks
  ...
  return cycleIssueDetails && "issueIds" in cycleIssueDetails ? (
    cycleIssueDetails.issueCount > 0 ? (
                ...
                {issue.target_date && (
                  <Tooltip label={`Target Date: ${renderFormattedDate(issue.target_date) ?? ""}`}>
                    <div className="flex h-full cursor-pointer items-center gap-1.5 truncate rounded-sm bg-layer-1 px-2 py-0.5 text-11 group-hover:bg-surface-1">
                      ...
                      <span className="truncate text-11">{renderFormattedDateWithoutYear(issue.target_date)}</span>
                    </div>
                  </Tooltip>
                )}
              </div>
      ...
      <div className="flex h-full w-full items-center justify-center">
        <SimpleEmptyState title={t("active_cycle.empty_state.priority_issue.title")} assetPath={priorityResolvedPath} />
      </div>
    )
  ) : (
    <ActiveCycleStatsLoader />
  );
});

type ActiveCycleDistributionStatsProps = Pick<ActiveCycleStatsProps, "cycle" | "handleFiltersUpdate"> & {
  emptyStateAssetPath: string;
};

const ActiveCycleAssigneeStats = observer(function ActiveCycleAssigneeStats(props: ActiveCycleDistributionStatsProps) {
  const { cycle, handleFiltersUpdate, emptyStateAssetPath: assigneesResolvedPath } = props;
  // plane hooks
  const { t } = useTranslation();

  return cycle && !isEmpty(cycle.distribution) ? (
    cycle?.distribution?.assignees && cycle.distribution.assignees.length > 0 ? (
        ...
        if (assignee.assignee_id)
          return (
            <SingleProgressStats
              ...
              onClick={() => {
                if (assignee.assignee_id) {
                  handleFiltersUpdate([{ property: "assignee_id", operator: "in", value: [assignee.assignee_id] }]);
                }
              }}
            />
          );
      ...
      <div className="flex h-full w-full items-center justify-center">
        <SimpleEmptyState title={t("active_cycle.empty_state.assignee.title")} assetPath={assigneesResolvedPath} />
      </div>
    )
  ) : (
    <ActiveCycleStatsLoader />
...
const ActiveCycleLabelStats = observer(function ActiveCycleLabelStats(props: ActiveCycleDistributionStatsProps) {
  const { cycle, handleFiltersUpdate, emptyStateAssetPath: labelsResolvedPath } = props;
  // plane hooks
  const { t } = useTranslation();
        ...
        <Tab.Panels as={Fragment}>
          ...
          >
            ...
            >
              <ActiveCyclePriorityIssues
                ...
                setIssueLoaderElement={setIssueLoaderElement}
                emptyStateAssetPath={priorityResolvedPath}
              />
            </div>
          </Tab.Panel>
          ...
          >
            <ActiveCycleAssigneeStats
              cycle={cycle}
              handleFiltersUpdate={handleFiltersUpdate}
              emptyStateAssetPath={assigneesResolvedPath}
            />
          </Tab.Panel>
          ...
          >
            <ActiveCycleLabelStats
              cycle={cycle}
              handleFiltersUpdate={handleFiltersUpdate}
              emptyStateAssetPath={labelsResolvedPath}
            />
          </Tab.Panel>
        </Tab.Panels>
      </Tab.Group>
